...
var cont = document.getElementsByClassName("cwow");
var arrleft = document.getElementsByClassName("arrleft");
var arrright = document.getElementsByClassName("arrright");
var ind = 1;
var i;

function sS(n) {
    if (n > cont.length) {ind = 1}    
    if (n < 1) {ind = cont.length}
    for (i = 0; i < cont.length; i++) {
        cont[i].className = cont[i].className.replace(" d-block", " d-none");
        cont[i].style.opacity = "0";
    }
    cont[ind-1].classList.remove("d-none");
    cont[ind-1].classList.add("d-block");
    cont[ind-1].style.opacity = "1";
}
